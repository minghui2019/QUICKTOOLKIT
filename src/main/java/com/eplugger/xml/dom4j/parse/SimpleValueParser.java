package com.eplugger.xml.dom4j.parse;
 ...
 * {@link com.eplugger.xml.dom4j.XMLObject#toBean(Class) 单个实例} (或
 * {@link com.eplugger.xml.dom4j.XMLObject#toBeans(String, Class) 实例列表}) 之前
 * {@link com.eplugger.xml.dom4j.parse.FieldValueParserFactory#reg(SimpleValueParser)
 * 手动注册}
 *
 * @param <T> 解析器支持的泛型类型
 * @version 1.3
...
public interface SimpleValueParser<T> {
	...
	/**
	 * 获取精准支持类型, 该类型可以加快查找速度. 如果返回null则需要通过 {@link #isSupport(Class)} 确认是否匹配.
	 *
	 * @return 精准支持类型
	 */
